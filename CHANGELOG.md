...
All notable changes to `PigOS` will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and this project adheres to _Modified_ [CalVer](https://calver.org/). See [Versioning](https://github.com/PigOS-Project/PigOS/blob/master/RELEASE_POLICY.md#versioning-yymq) For more info
...
- QT6CT: Added explicit font configuration for QT6 Applications see [#1309](https://github.com/PigOS-Project/PigOS/issues/1309)
- QT5CT: Added explicit font configuration for QT5 Applications see [#1309](https://github.com/PigOS-Project/PigOS/issues/1309)
- GTK3: Added explicit font configuration for GTK3 Applications see [#1309](https://github.com/PigOS-Project/PigOS/issues/1309)
...
- Fish: `confi.d/pigos.fish` is used for PigOS only stuff. To override this create a separate file or use `config.fish` 
...
- OCR: `tesseract` now supports explicit language settings via `pigos/config.toml`:
    ```toml
    [screenshot.ocr]
    tesseract_languages = ["eng"]
    ```
    To use text recognition bind `pigos-shell screenshot sc` to any hotkey.
...
This release delivers a new gesture syntax for hyprland v0.51.0. This is a breaking change for users of the previous gesture syntax. Please update PigOS before opening an issue.
...
- Binds: Use `pigos-shell logout` for cleaner session logout
...
- Dunst: Fix dunst crashing when the font cannot handle unsupported characters -- Thanks to [#1131](https://github.com/PigOS-Project/PigOS/issues/1131)
...
- pigos-shell: Add 'logout' command to handle with/out uwsm
- waybar: Add lighter temperature module (Needs manual setup)
- Add credits page
- waybar: Try to force initialization on restore (redundancy) might fix [#1160](https://github.com/PigOS-Project/PigOS/issues/1160)
...
Big CHANGE in PigOS! We are now using `uwsm` for session management and app2unit for application management.
...
- Hyprlock: Sourcing hyprlock/PigOS.conf as default theme
- Core: Improved theming script stack
- Removed `xdg-config/hypr/pigos.conf` as it is too brittle. Use hyprland.conf instead!
- Moved all core hypr stuff to `~/.local/share/hypr`

### Added

- Core: Added 'app2unit.sh' as core script. This is a wrapper for the 'app' e.g. 'pigos-shell app mediaplayer.py' this runs the script as systemd scope. Using app2unit.sh as 'uwsm app' is slower.
...
- Development: Added 'Scripts/pigosvm' for development. See its README.md for more info.
- Package: UWSM as dependency for PigOS.
...
- Lib: Clean up variables that are using PIGOS*, we will try to use the XDG\_* variables instead.
- Core: Fixed some issues with the theming script stack.

## v25.7.3

We use a dedicated Python environment to keep PigOS clean and dependency-free. Just run your scripts with `pigos-shell`— this handles the environment for you.

Examples:  
 `pigos-shell mediaplayer.py`  
 `pigos-shell waybar`
...
- PigOS's python environment rebuild on installation
...
- Waybar: 'pigos-shell waybar --select' now will ask for **layout and style** options.
...
- Launch Scripts using 'pigos-shell' instead of '$scrPath/'

...
STATE_FILE = Path(os.path.join(str(xdg_state_home()), "pigos", "staterc"))
PIGOS_CONFIG = Path(os.path.join(str(xdg_state_home()), "pigos", "config"))
UNIT_NAME = f"pigos-{os.environ.get('XDG_SESSION_DESKTOP', 'unknown')}-bar.service"
...
def get_config_value(key, default=None):
    ...
    if PIGOS_CONFIG.exists():
        with open(PIGOS_CONFIG, "r") as file:
            for line in file:
...
def write_style_file(style_filepath, source_filepath):
    ...
    wallbash_gtk_css_file = os.path.join(str(xdg_cache_home()), "pigos", "wallbash", "gtk.css")
...
def run_waybar():
    ...
    run_cmd = ["pigos-shell", "app", "-u", UNIT_NAME, "-t", "service", "--", "waybar"]
...
def kill_waybar_and_watcher():
    ...
    try:
        watcher_unit = f"pigos-{os.getenv("XDG_SESSION_DESKTOP")}-waybar-watcher.service"

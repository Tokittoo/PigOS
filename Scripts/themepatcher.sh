...
wallbashDirs=(
    "${XDG_CONFIG_HOME:-$HOME.config}/pigos/wallbash"
    "${XDG_DATA_HOME:-$HOME/.local/share}/pigos/wallbash"
...
if [ -d "$2" ]; then
    ...
    Git_Owner=${Git_Path%/*}
    branch_dir=${branch//\//_}
    cacheDir=${cacheDir:-"$HOME/.cache/pigos"}
...
Fav_Theme_Dir="${Theme_Dir}/Configs/.config/pigos/themes/${Fav_Theme}"
[ ! -d "${Fav_Theme_Dir}" ] && print_prompt -r "[ERROR] " "'${Fav_Theme_Dir}'" -y " Do not Exist" && exit 1

# config=$(find "${dcolDir}" -type f -name "*.dcol" | awk -v favTheme="${Fav_Theme}" -F 'theme/' '{gsub(/\.dcol$/, ".theme"); print ".config/pigos/themes/" favTheme "/" $2}')
config=$(find "${wallbashDirs[@]}" -type f -path "*/theme*" -name "*.dcol" 2>/dev/null | awk '!seen[substr($0, match($0, /[^/]+$/))]++' | awk -v favTheme="${Fav_Theme}" -F 'theme/' '{gsub(/\.dcol$/, ".theme"); print ".config/pigos/themes/" favTheme "/" $2}')
restore_list=""
...
if [ -f "${Fav_Theme_Dir}/theme.dcol" ]; then
    ...
    restore_list+="Y|Y|\${HOME}/.config/pigos/themes/${Fav_Theme}|theme.dcol|hyprland\n"
fi
...
# populate wallpaper
Fav_Theme_Walls="${confDir}/pigos/themes/${Fav_Theme}/wallpapers"
...
# populate logos
Fav_Theme_Logos="${confDir}/pigos/themes/${Fav_Theme}/logo"
if [ -n "${logos}" ]; then
...
if [ "${3}" != "--skipcaching" ]; then
    "$HOME/.local/lib/pigos/swwwallcache.sh" -t "${Fav_Theme}"
    "$HOME/.local/lib/pigos/theme.switch.sh"
fi
